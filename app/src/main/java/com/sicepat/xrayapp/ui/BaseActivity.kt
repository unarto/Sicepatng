...
import com.sicepat.xrayapp.helper.CustomDividerItemDecoration
...
abstract class BaseActivity : AppCompatActivity() {
    ...
    protected fun applyThemeSettings() {
        val isDark = Utils.getDarkModeStatus(this)
        val isPureBlack = com.sicepat.xrayapp.handler.MmkvManager.decodeSettingsBool(com.sicepat.xrayapp.AppConfig.PREF_THEME_PURE_BLACK, false)
        val customColorSeedHex = com.sicepat.xrayapp.handler.MmkvManager.decodeSettingsString(com.sicepat.xrayapp.AppConfig.PREF_THEME_COLOR_SEED, "#2196F3")
        
        val primaryColor = try {
            android.graphics.Color.parseColor(customColorSeedHex)
        } catch (e: Exception) {
        ...
        if (isDark && isPureBlack) {
            ...
            findViewById<View>(R.id.content_container)?.setBackgroundColor(android.graphics.Color.BLACK)
            findViewById<View>(R.id.toolbar)?.setBackgroundColor(android.graphics.Color.BLACK)
            window.statusBarColor = android.graphics.Color.BLACK
            window.navigationBarColor = android.graphics.Color.BLACK
        } else if (!isDark) {
            val bgDrawable = ContextCompat.getDrawable(this, R.drawable.bg_light_gradient)
            window.decorView.background = bgDrawable
            findViewById<View>(android.R.id.content)?.background = bgDrawable
            findViewById<View>(R.id.content_container)?.background = bgDrawable
            findViewById<View>(R.id.toolbar)?.setBackgroundColor(android.graphics.Color.TRANSPARENT)
            window.statusBarColor = android.graphics.Color.TRANSPARENT
            window.navigationBarColor = android.graphics.Color.TRANSPARENT
        }

        progressBar?.let {
            it.setIndicatorColor(primaryColor)
        }
        
        findViewById<com.google.android.material.appbar.MaterialToolbar>(R.id.toolbar)?.let { tb ->
            tb.navigationIcon?.setTint(primaryColor)
            tb.setTitleTextColor(if (isDark) android.graphics.Color.WHITE else android.graphics.Color.BLACK)
            tb.overflowIcon?.setTint(primaryColor)
        }
    }
