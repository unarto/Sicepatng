...
import com.sicepat.xrayapp.dto.ConfigResult
import com.sicepat.xrayapp.dto.CoreConfigContext
import com.sicepat.xrayapp.dto.V2rayConfig
import com.sicepat.xrayapp.dto.entities.ProfileItem
import com.sicepat.xrayapp.dto.entities.RulesetItem
import com.sicepat.xrayapp.enums.BalancerStrategyType
import com.sicepat.xrayapp.enums.CoreResolvedType
...
import com.sicepat.xrayapp.util.PackageUidResolver
...
object CoreConfigManager {
     ...
     */
    suspend fun getV2rayConfig(context: Context, guid: String): ConfigResult = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
        try {
            val configContext = CoreConfigContextBuilder.build(context, guid)
                ?: return@withContext ConfigResult(status = false, guid = guid, errorMessage = "Failed to build config context")
            if (configContext.isCustom) {
                return@withContext buildV2rayCustomConfig(configContext)
            }
            return@withContext toConfigResult(configContext, buildUnifiedConfig(configContext))
        } catch (e: Exception) {
     ...
     */
    suspend fun getV2rayConfig4Speedtest(context: Context, guid: String): ConfigResult = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
        try {
            val configContext = CoreConfigContextBuilder.build(context, guid)
                ?: return@withContext ConfigResult(status = false, guid = guid, errorMessage = "Failed to build config context")
            if (configContext.isCustom) {
            ...
            return@withContext toConfigResult(configContext, v2rayConfig)
        } catch (e: Exception) {
    ...
    private fun configureRouting(
        ...
        when (routingMode) {
            "1" -> { // Global
                val proxyTag = policyGroupBalancerTags[AppConfig.TAG_PROXY] ?: AppConfig.TAG_PROXY
