...
import com.sicepat.xrayapp.dto.OutboundTrafficStat
...
import com.sicepat.xrayapp.handler.NotificationManager
import com.sicepat.xrayapp.handler.SettingsManager
import com.sicepat.xrayapp.handler.SpeedtestManager
import com.sicepat.xrayapp.root.RootManager
import com.sicepat.xrayapp.service.CoreProxyOnlyService
import com.sicepat.xrayapp.service.CoreRootService
import com.sicepat.xrayapp.service.CoreVpnService
import com.sicepat.xrayapp.service.DialerNativeService
import com.sicepat.xrayapp.service.DialerWebviewService
import com.sicepat.xrayapp.service.IDialerService
import com.sicepat.xrayapp.util.LogUtil
import com.sicepat.xrayapp.util.MessageUtil
...
object CoreServiceManager {
    ...
    private var processFinderProxy: Any? = null
    ...
    var serviceControl: SoftReference<ServiceControl>? = null
        set(value) {
            ...
            if (service != null && processFinderProxy == null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                try { val cls = Class.forName("libv2ray.ProcessFinder"); processFinderProxy = java.lang.reflect.Proxy.newProxyInstance(cls.classLoader, arrayOf(cls), XrayProcessFinderHandler(service)) } catch (e: Exception) {}
                try {
                    val method = coreController.javaClass.getMethod("registerProcessFinder", Class.forName("libv2ray.ProcessFinder"))
                    method.invoke(coreController, processFinderProxy)
                } catch (e: Exception) {
                    LogUtil.e(AppConfig.TAG, "registerProcessFinder not supported", e)
                }
            }
        }
    ...
    private fun startContextService(context: Context) {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            if (isRootMode) {
                val hasRoot = kotlinx.coroutines.withContext(Dispatchers.IO) {
     ...
     */
    suspend fun startCoreLoop(vpnInterface: ParcelFileDescriptor?): Boolean = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
        if (coreController.isRunning) {
            LogUtil.w(AppConfig.TAG, "StartCore-Manager: Core already running")
    ...
    fun queryAllOutboundTrafficStats(): List<OutboundTrafficStat> {
        val payload = try {
            ...
            method.invoke(coreController) as String
        } catch (e: Exception) {
            ""
        }
...
//        LogUtil.d(AppConfig.TAG, "Queried outbound traffic stats: $result")
     ...
     */
    private class XrayProcessFinderHandler(val context: Context) : java.lang.reflect.InvocationHandler {
        override fun invoke(proxy: Any?, method: java.lang.reflect.Method, args: Array<out Any>?): Any? {
            if (method.name == "findProcessByConnection" && args != null && args.size >= 5) {
                return findProcessByConnection(args[0] as String, args[1] as String, args[2] as Long, args[3] as String, args[4] as Long)
            }
            return null
        }
        ...
        private fun findProcessByConnection(network: String, srcIP: String, srcPort: Long, destIP: String, destPort: Long): Long {
