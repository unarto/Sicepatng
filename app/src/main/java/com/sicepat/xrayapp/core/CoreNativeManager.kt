...
object CoreNativeManager {
    ...
    fun reconcileBrowserDialer(dialerAddr: String) {
        try {
            ...
            val method = clazz.getMethod("reconcileBrowserDialer", String::class.java)
            method.invoke(null, dialerAddr)
            LogUtil.i(AppConfig.TAG, "Browser dialer reconciled successfully with address: $dialerAddr")
        } catch (e: NoSuchMethodException) {
            LogUtil.d(AppConfig.TAG, "reconcileBrowserDialer not supported by this libv2ray version")
        } catch (e: Exception) {
