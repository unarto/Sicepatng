...
import com.sicepat.xrayapp.dto.entities.AssetUrlCache
import com.sicepat.xrayapp.dto.entities.AssetUrlItem
import com.sicepat.xrayapp.extension.concatUrl
...
class UserAssetViewModel : ViewModel() {
    ...
    private fun buildAssetList(
        ...
        // Force update URL for built-in geo files
        return (builtInItems + savedAssets).map { cache ->
            when (cache.assetUrl.remarks) {
                AppConfig.GEOIP_DAT -> cache.copy(
                    assetUrl = cache.assetUrl.copy(
                        url = "https://github.com/unarto/geoip/releases/download/202606250051/geoip.dat"
                    )
                )
                AppConfig.GEOSITE_DAT -> cache.copy(
                    assetUrl = cache.assetUrl.copy(
                        url = "https://github.com/unarto/geoip/releases/download/202606250051/geosite.dat"
                    )
                )
                AppConfig.GEOIP_ONLY_CN_PRIVATE_DAT -> cache.copy(
                    assetUrl = cache.assetUrl.copy(
